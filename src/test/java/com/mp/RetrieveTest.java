...
public class RetrieveTest {
    ...
    public void selectByIds() {
        List<Long> idsList = Arrays
                .asList(1094592041087729666L, 1088248166370832385L, 1087982257332887553L);
    ...
    /**
     * 2、名字中包含雨年并且龄大于等于20且小于等于40并且email不为空 name like '%雨%' and age between 20 and 40 and email is
     * not null
     */
    @Test
    ...
    /**
     * 4、创建日期为2019年2月14日并且直属上级为名字为王姓 date_format(create_time,'%Y-%m-%d')='2019-02-14' and manager_id
    ...
    /**
     * 10、名字中包含雨并且年龄小于40(需求1加强版) 第一种情况：select id,name from user where name like '%雨%' and age<40
    ...
    public void condition(String name, String email) {
        ...
        queryWrapper.like(StringUtils.isNotEmpty(name), "name", name)
                .like(StringUtils.isNotEmpty(email), "email",
                        email);
        List<User> userList = userMapper.selectList(queryWrapper);
        userList.forEach(System.out::println);
    }
    ...
    /**
     * 11、按照直属上级分组，查询每组的平均年龄、最大年龄、最小年龄。 并且只取年龄总和小于500的组。 select avg(age) avg_age,min(age)
     * min_age,max(age) max_age from user group by manager_id having sum(age) <500 {0}可变参数
     */
    @Test
    ...
    public void selectLambda3() {
        List<User> list = new LambdaQueryChainWrapper<User>(userMapper).like(User::getName, "name")
                .ge(User::getAge, 20)
                .list();
